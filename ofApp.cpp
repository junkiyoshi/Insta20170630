...
void ofApp::draw() {
	...
	for (int i = 0; i < triangles.size(); i++) {
		...
		float noise_value = ofNoise(avg.x * 0.0025, avg.y * 0.0025, avg.z * 0.0025, ofGetFrameNum() * 0.0025);
		...
		if (noise_value > 0.75) {
			...
			mesh.addVertex(triangles[i].getVertex(0) * ofMap(noise_value, 0.75, 1, 1, 3));
			mesh.addVertex(triangles[i].getVertex(1) * ofMap(noise_value, 0.75, 1, 1, 3));
			mesh.addVertex(triangles[i].getVertex(2) * ofMap(noise_value, 0.75, 1, 1, 3));
